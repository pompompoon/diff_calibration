...
def analyze_shizuoka_2023_data(base_dir, output_dir, metadata_path, sensitivity_threshold=15, velocity_threshold=30, exclude_initial_seconds=5):
    """
    ...
    sensitivity_threshold : int
        LessThanSensitivityの閾値（デフォルト：15）
    velocity_threshold : float
        サッカード検出の速度閾値 (deg/s)
    exclude_initial_seconds : float
        試験開始から除外する秒数
    ...
    # 方向転換の回数を格納するリスト（統計量計算用）
    direction_changes_list = []
    direction_change_freq_list = []
    ...
    for i, subject_dir in enumerate(subject_dirs, 1):
        ...
        # 感度チェック
        should_process, sensitivity_count = sensitivity_filter.check_result_file(subject_dir)
        
        if not should_process:
            print(f"\nスキップ ({i}/{len(subject_dirs)}): {subject_id}")
        ...
        for calib_file in calib_files:
            try:
                ...
                if 'direction_changes' in results:
                    direction_changes_list.append(results['direction_changes'])
                if 'direction_change_frequency' in results:
                    direction_change_freq_list.append(results['direction_change_frequency'])
    ...
    direction_change_freq_stats = {}
    
    if direction_changes_list:
        direction_changes_stats = {
            'direction_changes_mean': np.mean(direction_changes_list),
            'direction_changes_std': np.std(direction_changes_list),
            'direction_changes_min': np.min(direction_changes_list),
            'direction_changes_max': np.max(direction_changes_list),
            'direction_changes_median': np.median(direction_changes_list)
        }
    
    if direction_change_freq_list:
        direction_change_freq_stats = {
            'direction_change_freq_mean': np.mean(direction_change_freq_list),
            'direction_change_freq_std': np.std(direction_change_freq_list),
            'direction_change_freq_min': np.min(direction_change_freq_list),
            'direction_change_freq_max': np.max(direction_change_freq_list),
            'direction_change_freq_median': np.median(direction_change_freq_list)
        }
    ...
    if all_results:
        ...
        cols = ['InspectionDateAndId', 'less_than_sensitivity_count', 'file_name'] + [col for col in combined_results.columns 
                                                     if col not in ['InspectionDateAndId', 'less_than_sensitivity_count', 'file_name']]
        combined_results = combined_results[cols]
        
        # CSVファイルとして保存（感度閾値を含むファイル名）
        output_file = os.path.join(output_dir, f'shizuoka2023_EyeCenterAngle_saccade_analysis_sens{sensitivity_threshold}_vel{velocity_threshold}_excl{exclude_initial_seconds}s_freqbands.csv')
        combined_results.to_csv(output_file, index=False, encoding='utf-8')
        
        # 方向転換の統計量を別ファイルとして保存
        stats_dict = {**direction_changes_stats, **direction_change_freq_stats}
        stats_df = pd.DataFrame([stats_dict])
        stats_file = os.path.join(output_dir, f'direction_change_stats_sens{sensitivity_threshold}_vel{velocity_threshold}_excl{exclude_initial_seconds}s.csv')
        stats_df.to_csv(stats_file, index=False, encoding='utf-8')
        
        # スキップされたフォルダの情報を保存
        if skipped_folders:
            skipped_df = pd.DataFrame(skipped_folders)
            skipped_file = os.path.join(output_dir, f'skipped_folders_sens{sensitivity_threshold}.csv')
        ...
        # 方向転換の統計量を表示
        if direction_changes_stats:
            print("\n方向転換の統計量:")
            print(f"- 回数 平均: {direction_changes_stats['direction_changes_mean']:.2f} ± {direction_changes_stats['direction_changes_std']:.2f}")
            print(f"- 頻度 平均: {direction_change_freq_stats['direction_change_freq_mean']:.4f} ± {direction_change_freq_stats['direction_change_freq_std']:.4f} 回/ms")
        
        print(f"\n保存先:")
        print(f"- 分析結果: {output_file}")
        print(f"- 方向転換統計: {stats_file}")
        if skipped_folders:
