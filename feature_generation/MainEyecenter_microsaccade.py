...
class SaccadeAnalyzer:
    ...
    def detect_microsaccades(self, velocity_threshold_low=10, velocity_threshold_high=100, 
        ...
        # 発生頻度が範囲内かチェック（オプション）
        # frequency_in_range = 0.2 <= microsaccade_frequency <= 3.0
...
def analyze_multiple_calibrations(base_dir, output_dir, sensitivity_threshold=15, velocity_threshold=30, exclude_initial_seconds=5):
    ...
    if all_results:
        ...
        output_file = os.path.join(output_dir, f'combined_EyeCenterAngle_saccade_analysis_sens{sensitivity_threshold}_vel{velocity_threshold}_excl{exclude_initial_seconds}s_angle_micro.csv')
