...
class SaccadeAnalyzer:
    ...
    def calculate_direction_changes(self, angle_threshold=np.pi/4):
        """
        方向転換の回数を計算し、各15秒間ウィンドウにおける回数を算出
        
        Parameters:
        -----------
        angle_threshold : float
            方向転換とみなす角度変化の閾値（ラジアン）
        """
        ...
        # ウィンドウサイズ（15秒 = 15000ミリ秒）
        window_size_ms = 15000
        
        # 各15秒ウィンドウの方向転換回数を格納するリスト
        window_direction_changes = []
        window_start_times = []
        
        # 各15秒ウィンドウごとに方向転換回数を計算
        ...
        while current_window_start < end_time:
            ...
            # このウィンドウに含まれるデータポイントのインデックスを取得
            window_indices = np.where(
                (self.df['timestamp'].values >= current_window_start) & 
                (self.df['timestamp'].values < current_window_end)
            )[0]
            
            # このウィンドウにデータが存在するか確認
            if len(window_indices) > 1:  # 少なくとも2点必要（角度差計算のため）
                # このウィンドウに対応する角度差インデックスを取得
                # 連続したデータポイント間の角度差のみを考慮
                window_angle_diff_indices = []
                
                for i in range(len(window_indices) - 1):
                    if window_indices[i] + 1 == window_indices[i+1]:  # 連続したインデックスの場合のみ
                        idx = window_indices[i]
                        if idx < len(angle_diff):  # 角度差配列の範囲内にあるか確認
                            window_angle_diff_indices.append(idx)
                
                # このウィンドウ内の方向転換回数をカウント
                if window_angle_diff_indices:
                    window_changes = np.sum(np.abs(angle_diff[window_angle_diff_indices]) > angle_threshold)
                    window_direction_changes.append(window_changes)
                    window_start_times.append(current_window_start)
                    
                    # デバッグ用に表示
                    window_start_sec = (current_window_start - start_time) / 1000
                    window_end_sec = (current_window_end - start_time) / 1000
                    print(f"ウィンドウ {window_number}: {window_start_sec:.1f}～{window_end_sec:.1f}秒, 方向転換回数: {window_changes}")
            
            # 次のウィンドウの開始時間に移動
            current_window_start = current_window_end
        
        # 全体の方向転換回数（比較用）
        total_direction_changes = np.sum(np.abs(angle_diff) > angle_threshold)
        ...
        # 全体の方向転換頻度（1秒あたり）
        direction_change_freq = total_direction_changes / total_time_sec if total_time_sec > 0 else 0
        
        # 各ウィンドウの方向転換回数の統計量
        if window_direction_changes:
            windows_mean = np.mean(window_direction_changes)
            windows_std = np.std(window_direction_changes)
            windows_min = np.min(window_direction_changes)
            windows_max = np.max(window_direction_changes)
        else:
            windows_mean = np.nan
            windows_std = np.nan
            windows_min = np.nan
        ...
        return {
            'direction_changes_total': total_direction_changes,
            'direction_change_frequency': direction_change_freq,
            'window_15sec_direction_changes': str(window_direction_changes),  # 各ウィンドウの値をCSVに保存
            'window_15sec_start_times': str(window_start_times),  # 各ウィンドウの開始時間をCSVに保存
            'window_15sec_count': len(window_direction_changes),  # ウィンドウ数
            'window_15sec_direction_changes_mean': windows_mean,  # 平均
            'window_15sec_direction_changes_std': windows_std,    # 標準偏差
            'window_15sec_direction_changes_min': windows_min,    # 最小値
            'window_15sec_direction_changes_max': windows_max     # 最大値
        }
...
def analyze_eye_movements(input_file, output_dir, velocity_threshold=30, exclude_initial_seconds=5):
    """
    眼球運動データを分析する関数
    ...
    analyzer.plot_saccade_trajectories(output_dir, base_name, velocity_threshold=velocity_threshold)
    analyzer.plot_profiles(output_dir, base_name)
    """
    return results
    

def analyze_multiple_calibrations(base_dir, output_dir, sensitivity_threshold=1, velocity_threshold=30, exclude_initial_seconds=0):
    """
    ...
    # 各ファイルの15秒ウィンドウの方向転換統計量を格納するリスト
    all_window_means = []
    all_window_stds = []
    ...
    for i, folder in enumerate(folders, 1):
        ...
        for calib_file in calib_files:
            try:
                ...
                print(f"ファイル: {os.path.basename(calib_file)}")
                ...
                # 各ファイルの15秒ウィンドウ統計量を収集
                if 'window_15sec_direction_changes_mean' in results and not np.isnan(results['window_15sec_direction_changes_mean']):
                    all_window_means.append(results['window_15sec_direction_changes_mean'])
                if 'window_15sec_direction_changes_std' in results and not np.isnan(results['window_15sec_direction_changes_std']):
                    all_window_stds.append(results['window_15sec_direction_changes_std'])
    ...
    # 全ファイルの15秒ウィンドウ統計量の平均と標準偏差
    window_stats = {}
    if all_window_means and all_window_stds:
        window_stats = {
            'all_files_window_means_mean': np.mean(all_window_means),
            'all_files_window_means_std': np.std(all_window_means),
            'all_files_window_stds_mean': np.mean(all_window_stds),
            'all_files_window_stds_std': np.std(all_window_stds)
        }
    ...
    if all_results:
        ...
        cols = ['InspectionDateAndId', 'file_name', 'less_than_sensitivity_count'] + [col for col in combined_results.columns if col not in ['InspectionDateAndId', 'file_name', 'less_than_sensitivity_count']]
        combined_results = combined_results[cols]
        
        # CSVファイルとして保存（閾値と除外時間を含むファイル名）
        output_file = os.path.join(output_dir, f'combined_15sec_windows_analysis_sens{sensitivity_threshold}_vel{velocity_threshold}_excl{exclude_initial_seconds}s.csv')
        combined_results.to_csv(output_file, index=False, encoding='utf-8')
        
        # 全ファイルの15秒ウィンドウ統計量を別ファイルとして保存
        if window_stats:
            stats_df = pd.DataFrame([window_stats])
            stats_file = os.path.join(output_dir, f'all_files_15sec_window_stats_sens{sensitivity_threshold}_vel{velocity_threshold}.csv')
        ...
        # 全ファイルの15秒ウィンドウ統計量を表示
        if window_stats:
            print("\n全ファイルの15秒ウィンドウ統計量:")
            print(f"- 各ファイルの方向転換平均値の平均: {window_stats['all_files_window_means_mean']:.2f}")
            print(f"- 各ファイルの方向転換標準偏差の平均: {window_stats['all_files_window_stds_mean']:.2f}")
        
        print(f"\n保存先:")
        print(f"- 分析結果: {output_file}")
        if window_stats:
            print(f"- 全ファイルの15秒ウィンドウ統計量: {stats_file}")
        if skipped_folders:
...
if __name__ == "__main__":
    ...
    velocity_threshold = 0         # サッカード検出の速度閾値 (deg/s)
