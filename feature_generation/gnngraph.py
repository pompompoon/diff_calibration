...
def create_gaze_graph(fixations, df):
    ...
    for i in range(len(fixations) - 1):
        ...
        # サッカードの指標計算
        saccade_distance = euclidean([current_fixation['x'], current_fixation['y']], 
                                    [next_fixation['x'], next_fixation['y']])
        saccade_duration = next_fixation['start_time'] - current_fixation['end_time']
        
        # 注視点間に時間差がある場合のみエッジを追加
        if saccade_duration > 0:
            saccade_velocity = saccade_distance / (saccade_duration / 1000)  # 単位/秒
...
def plot_gaze_graph(G, fixations, output_file=None):
    ...
    if edges:
        ...
        # エッジラベルの描画
        edge_labels = {(u, v): f"v={G[u][v]['velocity']:.1f}" for u, v in edges}
...
def main(file_path=file_path, output_dir=output_dir):
    ...
    return G, fixations, nodes_df, edges_df

if __name__ == "__main__":
    G, fixations, nodes_df, edges_df = main()