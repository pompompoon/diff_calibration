...
def prepare_data_for_regression_with_smote(df, target_column='target', use_smote=False, 
                                           smote_method='density', use_integer_smote=True,
                                           target_min=9, target_max=30, **smote_kwargs):
    """
    データの前処理を行う関数（整数値対応回帰用SMOTE付き）
    ...
    **smote_kwargs : dict
    ...
    target : pandas.Series
        目的変数
    id_column : pandas.Series or None
        ID列（存在する場合）
    """
    print("\nデータの形状:", df.shape)
    ...
    # SMOTEを適用する場合
    if use_smote:
        print(f"\n{'整数値対応' if use_integer_smote else ''}回帰用SMOTE（{smote_method}）を適用します...")
        
        # 整数値対応SMOTEまたは通常のSMOTEを選択
        if use_integer_smote:
            # 目的変数の範囲をチェック
            if target.min() < target_min or target.max() > target_max:
                print(f"警告: 目的変数の範囲（{target.min()}-{target.max()}）が指定範囲（{target_min}-{target_max}）を超えています")
            
            # 新しいIntegerRegressionSMOTEクラスのインポート
            try:
                from regression_smote import IntegerRegressionSMOTE
                smote = IntegerRegressionSMOTE(
                    method=smote_method, 
                    k_neighbors=5, 
                    random_state=42,
                ...
                # フォールバック：手動で整数化を実装
                print("IntegerRegressionSMOTEが見つかりません。通常のSMOTEで整数化を適用します...")
                from regression_smotebackup import RegressionSMOTE
                smote = RegressionSMOTE(method=smote_method, k_neighbors=5, random_state=42)
                use_integer_smote = False  # フォールバック処理を有効にする
        else:
            from regression_smotebackup import RegressionSMOTE
            smote = RegressionSMOTE(method=smote_method, k_neighbors=5, random_state=42)
        
        # リサンプリングを実行
        X_resampled, y_resampled = smote.fit_resample(features.values, target.values, **smote_kwargs)
        
        # フォールバック処理：通常のSMOTEを使った場合の整数化
        if not use_integer_smote or 'IntegerRegressionSMOTE' not in str(type(smote)):
            print("合成データの整数化を実行中...")
            n_original = len(target)
            
            # 合成されたサンプルのみ整数化
            for i in range(n_original, len(y_resampled)):
                # 四捨五入で整数化
                y_resampled[i] = round(y_resampled[i])
                # 範囲制限
                y_resampled[i] = np.clip(y_resampled[i], target_min, target_max)
            
            # 整数化の確認
            synthetic_targets = y_resampled[n_original:]
        ...
        # 整数化の最終確認
        if use_integer_smote or 'integer' in smote_method.lower():
            n_original = len(target)
            synthetic_targets = y_resampled[n_original:]
            non_integer_count = np.sum(synthetic_targets % 1 != 0)
            if non_integer_count > 0:
        ...
        target_resampled = pd.Series(y_resampled, name=target.name)
        
        # ID列の処理
        if id_column is not None:
            original_ids = id_column.values
            n_synthetic = len(X_resampled) - len(original_ids)
            
            # 合成サンプル用の新しいID生成
            synthetic_ids = [f"synthetic_{i:06d}" for i in range(n_synthetic)]
            id_resampled = pd.Series(list(original_ids) + synthetic_ids, name=id_column.name)
        else:
            id_resampled = None
        
        print(f"SMOTE適用後のデータ数: {len(features_resampled)} (元: {len(features)}, 追加: {len(X_resampled) - len(features)})")
        print(f"データ増加率: {((len(X_resampled) - len(features)) / len(features) * 100):.2f}%")
        
        # SMOTEの効果を可視化（デバッグモード時）
            ...
            try:
                ...
                if use_integer_smote:
                    try:
                        ...
                        visualize_integer_smote_effect(features.values, target.values, X_resampled, y_resampled)
                    except ImportError:
                        from regression_smotebackup import visualize_smote_effect
                        visualize_smote_effect(features.values, target.values, X_resampled, y_resampled)
                else:
                    from regression_smotebackup import visualize_smote_effect
                    visualize_smote_effect(features.values, target.values, X_resampled, y_resampled)
        ...
        return features_resampled, target_resampled, id_resampled
...
def run_regression_analysis(df, model_class=None, use_bagging=False, 
    ...
    if use_smote:
        print(f"回帰用SMOTE（{smote_method}）を適用します。")
        print(f"SMOTEパラメータ: {smote_kwargs}")
    
    # ============================================
    # 4. dfの前処理（dfがメイン処理される部分）
    # ============================================
    ...
    try:
        # SMOTE対応の前処理関数を使用
        # ここでdfが実際に処理される
        features, target, id_column = prepare_data_for_regression_with_smote(
            df,  # ← dfパラメータがここで渡される
        ...
        print(f"前処理後の特徴量形状: {features.shape}")
        print(f"前処理後のターゲット形状: {target.shape}")
        if id_column is not None:
            print(f"ID列の情報: {id_column.name}, 長さ: {len(id_column)}")
            
    except TypeError as e:
        # 古い関数を使用する場合のフォールバック
        print("警告: SMOTE対応の前処理関数が使用できません。通常の前処理に戻します...")
        print(f"エラー詳細: {e}")
        
        # 通常の前処理を実行（dfはここでも使用される）
        features, target, id_column = prepare_data_for_regression(df, target_column=target_column)
        
        print(f"通常前処理後の特徴量形状: {features.shape}")
        print(f"通常前処理後のターゲット形状: {target.shape}")
        
        # SMOTEを手動で適用（dfから生成されたfeaturesとtargetを使用）
        if use_smote:
            try:
                from regression_smotebackup import RegressionSMOTE
                print(f"手動でSMOTE（{smote_method}）を適用します...")
                smote = RegressionSMOTE(method=smote_method, k_neighbors=5, random_state=random_state)
                X_resampled, y_resampled = smote.fit_resample(features.values, target.values, **smote_kwargs)
                
                # データフレームに戻す
                features = pd.DataFrame(X_resampled, columns=features.columns)
                target = pd.Series(y_resampled, name=target.name)
                ...
                if id_column is not None:
                    ...
                    n_synthetic = len(X_resampled) - len(original_ids)
                    synthetic_ids = [f"synthetic_{i:06d}" for i in range(n_synthetic)]
                    id_column = pd.Series(list(original_ids) + synthetic_ids, name=id_column.name)
                
                # SMOTE効果をログ出力
                original_size = len(df)  # 元のdfサイズ
                resampled_size = len(features)
                print(f"SMOTE適用後のデータ数: {resampled_size}")
                print(f"元のdfサイズ: {original_size}")
                print(f"追加されたサンプル: {resampled_size - original_size}")
                print(f"データ増加率: {((resampled_size - original_size) / original_size * 100):.2f}%")
    ...
    if use_smote:
        ...
        print(f"最終データ数: {len(features)}")
        print(f"データ増加: {len(features) - len(df)}")
    ...
    if generate_pdp:
        ...
        try:
            ...
            if output_dir:
                ...
                plt.close(fig)
                print(f"統合部分依存プロットを保存しました: {pdp_path}")
            else:
                plt.show()
            ...
            if len(top_features) >= 2:
                ...
                if output_dir:
                    ...
                    plt.close(fig)
                    print(f"特徴量相互作用プロットを保存しました: {interaction_path}")
                else:
                    plt.show()
            
            # 3. SMOTE効果と部分依存の比較分析（SMOTE使用時のみ）
            if use_smote and 'X_train_orig' in results:
                print("\nSMOTE適用前後の部分依存プロット比較...")
                print("元のdfデータでモデルを学習中...")
                ...
                try:
                    ...
                    if use_bagging:
                        ...
                        original_model.fit(results['X_train_orig'], results['y_train'])
                    else:
                    ...
                    axes[0].set_title(f"元データ（df）: {top_feature}", fontsize=12)
                    ...
                    if output_dir:
                        ...
                        plt.close(fig)
                        print(f"SMOTE効果比較プロットを保存しました: {comparison_path}")
                    else:
                        plt.show()
            ...
            try:
                ...
                if output_dir:
                    ...
                    plt.close(fig)
                    print(f"バックアップ部分依存プロットを保存しました: {backup_path}")
                else:
                    plt.show()
        ...
        try:
            with open(config_path, 'w') as f:
                ...
                if use_smote:
                    ...
                    resampled_size = len(features)
                    f.write(f"\n=== データサイズ変化 ===\n")
                    f.write(f"元のdfデータ数: {original_size}\n")
                    f.write(f"SMOTE適用後: {resampled_size}\n")
                    f.write(f"合成データ数: {resampled_size - original_size}\n")
                    f.write(f"データ増加率: {((resampled_size - original_size) / original_size * 100):.2f}%\n")
            ...
            print(f"実行設定を保存しました: {config_path}")
        except Exception as e:
    ...
    if output_dir and data_file_name:
        ...
        try:
            # dfから派生した最終的な特徴量とターゲットを使ってモデル保存
            save_regression_model(
                model=results['model'],
                features=features,  # dfから前処理されたSMOTE適用後の特徴量を使用
                target=target,      # dfから前処理されたSMOTE適用後のターゲットを使用
    ...
    print(f"\n=== 分析完了レポート ===")
    print(f"分析が完了しました。すべての結果は {output_dir} に保存されました。")
    ...
    if use_smote:
        ...
        synthetic_size = final_size - original_size if final_size > original_size else 0
        
        print(f"\nSMOTE効果:")
        print(f"  - 元データサイズ: {original_size}")
        print(f"  - 最終データサイズ: {final_size}")
        print(f"  - 合成サンプル数: {synthetic_size}")
        print(f"  - データ増加率: {((final_size - original_size) / original_size * 100):.2f}%")
    ...
    results['df_info'] = {
        ...
        'data_increase_ratio': ((len(features) - len(df)) / len(df) * 100) if use_smote else 0
    }
